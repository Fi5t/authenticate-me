...
class HiddenEditText @JvmOverloads constructor(
    ...
    init {
        ...
        try {
            attrsArray = context.obtainStyledAttributes(attrs, R.styleable.HiddenEditText)
            with(attrsArray) {
                spaceBetween = getDimensionPixelSize(R.styleable.HiddenEditText_spaceBetween, -1)
                itemWidth = getDimensionPixelSize(R.styleable.HiddenEditText_itemWidth, -1)
                itemHeight = getDimensionPixelSize(R.styleable.HiddenEditText_itemHeight, -1)
                emptyDrawable = getResourceId(R.styleable.HiddenEditText_emptyDrawable, -1)
                filledDrawable = getResourceId(R.styleable.HiddenEditText_filledDrawable, -1)
                length = getInteger(R.styleable.HiddenEditText_length, DEFAULT_CODE_LENGTH)
            }
        } finally {
            attrsArray.recycle()
        }
    ...
    private fun validateAttrs() {
        require(spaceBetween != -1) { "HiddenEditText: spaceBetween should be set" }
        require(itemWidth != -1) { "HiddenEditText: itemWidth should be set" }
        require(itemHeight != -1) { "HiddenEditText: itemHeight should be set" }
        require(emptyDrawable != -1) { "HiddenEditText: emptyDrawable should be set" }
        require(filledDrawable != -1) { "HiddenEditText: filledDrawable should be set" }
    }
}
